...
{
  //Check the sensor for new data for 200ms
  if(safeCheck(200)) // 200ms
    return (sense.IR[sense.head]);
  else
    return(0); //Sensor failed to find new data
}

void MAX30105::getIRandRed(int32_t *ir_val, int32_t *red_val) {
  if(safeCheck(200)) {
    *ir_val = (int)sense.IR[sense.head];
    *red_val = (int)sense.red[sense.head];
  } else {
    *ir_val = 0;
    *red_val = 0;
  }
...
bool MAX30105::safeCheck(uint8_t maxTimeToCheck)
{
  ...
  while(1) {
	  ...
	  if(check() == true) return(true);//We found new data!
